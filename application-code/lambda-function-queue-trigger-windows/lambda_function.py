...
def lambda_handler(event, context):
    ...
    response = ssm.get_parameters(
        Names=[
            'WIN_PIPELINE_UNPROCESSED_SQS_URL',
            'WIN_PIPELINE_ENABLED',
            'WIN_PIPELINE_ECS_MAX_TASKS',
            'WIN_PIPELINE_ECS_CLUSTER',
            'WIN_PIPELINE_ECS_TASK_CONTAINER',
            'WIN_PIPELINE_ECS_TASK_DEFINITON',
            'WIN_PIPELINE_ECS_TASK_SECURITYGROUP',
            'WIN_PIPELINE_ECS_TASK_SUBNET',
            'WIN_PIPELINE_S3_DEST_PREFIX'
        ],
        WithDecryption=True
    )
    ...
    for param in params:
        if param['Name'] == 'WIN_PIPELINE_UNPROCESSED_SQS_URL':
            sqs_url = param['Value']
        if param['Name'] == 'WIN_PIPELINE_ECS_MAX_TASKS':
            max_tasks = param['Value']
        if param['Name'] == 'WIN_PIPELINE_ENABLED':
            pipeline_enabled = param['Value']
        if param['Name'] == 'WIN_PIPELINE_ECS_CLUSTER':
            TASK_CLUSTER = param['Value']
        if param['Name'] == 'WIN_PIPELINE_ECS_TASK_CONTAINER':
            TASK_CONTAINER = param['Value']
        if param['Name'] == 'WIN_PIPELINE_ECS_TASK_DEFINITON':
            taskdef = param['Value']
            TASK_DEFINITON = taskdef[:taskdef.rindex(':')]
        if param['Name'] == 'WIN_PIPELINE_ECS_TASK_SUBNET':
            TASK_SUBNET = param['Value']
        if param['Name'] == 'WIN_PIPELINE_ECS_TASK_SECURITYGROUP':
            TASK_SECURITYGROUP = param['Value']
        if param['Name'] == 'WIN_PIPELINE_S3_DEST_PREFIX':
